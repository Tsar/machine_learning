...
if __name__ == "__main__":
    ...
    with open("prices.txt", "r") as f:
        for line in f:
            ...
            if line != "":
                pricesOrig.append([float(val) for val in line.split(",")[0:N]])

    minVal = []
    maxVal = []
    for i in range(N):
        minVal.append(min([row[i] for row in pricesOrig]))
        maxVal.append(max([row[i] for row in pricesOrig]))

    for rowOrig in pricesOrig:
        row = []
        for i in range(N):
            row.append((rowOrig[i] - minVal[i]) / (maxVal[i] - minVal[i]))
        prices.append(row)

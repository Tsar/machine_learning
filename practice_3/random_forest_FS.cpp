/** Random forest from practice_2 modified for working with files processed by feature selection **/
...
void readData(std::string const& fileName, int& featuresCount, std::vector<int>* data) {
    FILE* f = fopen(fileName.c_str(), "r");
    fscanf(f, "%d", &featuresCount);
    for (int i = 0; i < 100; ++i) {
...
public:
        ...
        , fIndex(-1)
    {
        ...
        for (int i = 0; i < sqrt(fCount); ++i) {
            featureIndexes.push_back(rand() % fCount);
        }
...
int main() {
    ...
    readData("arcene_train_FS.data", fCount, trainData);
    readLabels("../practice_2/random_forest/arcene_train.labels", trainLabels);
    ...
    int testLabels[100];
    readData("arcene_valid_FS.data", fCount2, testData);
    if (fCount != fCount2) {
        printf("ERROR: Unequal number of features in train and test data!\n");
        return 1;
    }
    readLabels("../practice_2/random_forest/arcene_valid.labels", testLabels);
